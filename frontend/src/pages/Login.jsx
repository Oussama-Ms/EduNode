...
const Login = ({ setAuth }) => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex items-center justify-center relative overflow-hidden max-w-[100vw] overflow-x-hidden">
      {/* Light/Dark Theme Blobs */}
      <div className="absolute top-[-10%] left-[-10%] w-96 h-96 bg-brand-400 dark:bg-brand-600 rounded-full mix-blend-multiply dark:mix-blend-screen filter blur-[128px] opacity-30 dark:opacity-20 animate-blob pointer-events-none"></div>
      <div className="absolute bottom-[-10%] right-[-5%] w-72 h-72 bg-blue-400 dark:bg-blue-600 rounded-full mix-blend-multiply dark:mix-blend-screen filter blur-[128px] opacity-30 dark:opacity-20 animate-blob animation-delay-2000 pointer-events-none"></div>
      ...
      <div className="glass-card w-full max-w-md p-8 relative z-10 animate-fade-in text-slate-900 dark:text-slate-100">
        <div className="text-center mb-8">
          ...
          <p className="text-slate-500 dark:text-slate-400 mt-2 text-sm uppercase tracking-widest font-semibold">Admin Portal</p>
        </div>
        ...
        <p className="mt-6 text-center text-sm text-slate-500 dark:text-slate-400">
          Don't have an account? <Link to="/register" className="text-brand-600 dark:text-brand-400 hover:text-brand-500 dark:hover:text-brand-300 transition-colors font-medium">Create one</Link>
        </p>
      </div>
    </div>
  );
